...
transform slot_zoom:
    subpixel True # Делает анимацию кристально плавной
    transform_anchor True # Важно: зум не будет менять размер контейнера
    on hover:
        ease 0.2 zoom 1.05
    on idle:
        ease 0.2 zoom 1.0
...
screen quick_menu():
    ...
    if quick_menu:
        ...
        hbox:
            ...
            textbutton _("Альманах") action ShowMenu('diary')
...
screen main_menu():
    ...
    if gui.show_name:
        ...
        vbox:
            ...
            # 1. Горизонтальный ряд ТОЛЬКО для иконки и названия
            hbox:
                spacing 15
                yalign 0.5
...
screen about():
    ...
    use game_menu(_("Об игре"), scroll="viewport"):
        ...
        vbox:
            ...
            text _("Этерия — мир, забытый своим создателем. \n\nСын опального профессора и беглый исследователь с дырой в памяти — последняя преграда на пути Инквизиции к полному контролю над сущностями. \n\nСможете ли вы пробудить искру в мире, где остался только пепел?"):
                style "about_text"
                # Если хочешь сдвинуть или выровнять:
            text _("Авторы проекта: Andrei Korolev, Artem Sorochan, Vlad Kluchko \nСюжет и Код:Andrei Korolev, Roman Sydorenko\n\nСделано на движке Ren'Py. 2026 год."):
                style "about_text"
        ...
        fixed:
            ...
            grid gui.file_slot_cols gui.file_slot_rows:
                ...
                for i in range(gui.file_slot_cols * gui.file_slot_rows):
                    ...
                    fixed:
                        area (0, 0, gui.slot_button_width, gui.slot_button_height)
...
screen diary():
    ...
    textbutton "Сбросить прогресс":
        action Function(persistent.unlocked_entities.clear)
        align (0.95, 0.95) # В нижний правый угол

    # Основной контейнер смещаем выше (yalign 0.2 вместо 0.4)
    vbox:
        align (0.5, 0.2)
        spacing 30
        ...
        frame:
            ...
            vpgrid:
                ...
                for key in lore_data:
                    $ name, desc, img = lore_data[key]
                    $ is_unlocked = key in persistent.unlocked_entities

                    # ФИКС СДВИГА: оборачиваем в fixed
                    fixed:
                        # Задаем жесткий размер области, чтобы зум не выходил за рамки для грида
                        area (0, 0, 200, 240)

                        button:
                            background None
                            at slot_zoom
                            # Центрируем внутри fixed, чтобы зум шел во все стороны равномерно
                            align (0.5, 0.5)
                            action (SetVariable("current_lore", key) if is_unlocked else None)
                            ...
                            vbox:
                                ...
                                text (name if is_unlocked else "???"):
                                    xalign 0.5
                                    size 22
                                    color ("#ffaa00" if is_unlocked else "#666")

    # Описание остается снизу
    if current_lore:
        frame:
            align (0.5, 0.95)
            xfill True
            ...
            vbox:
                ...
                text lore_data[current_lore][0]:
                    size 32
                    color "#ffaa00"
                    xalign 0.5

                text lore_data[current_lore][1]:
                    size 22
                    text_align 0.5
                    xalign 0.5
                    italic True